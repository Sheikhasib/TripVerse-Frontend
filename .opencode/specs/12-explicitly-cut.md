...
Settings page (distinct from profile — the sidebar's "Settings-lite" just reuses profile fields), Categories manager page, Terms page (real content), contact/admin inbox UI, notifications, blog comments, review edit/delete, email verification, password reset, Google OAuth extra flows (server handles linking). All are additive later — none require touching MVP code to bolt on.

Payment was in this cut list; it's been promoted to `13-payment-gateway.md` (SSLCommerz, backend Step 16), along with `/user-dashboard/payments`. The wishlist has been built from `16-wishlist.md`.
